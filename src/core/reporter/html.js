...
export async function exportHtml(url, baseDir) {
  ...
  for (const f of sorted) {
    if (f.module !== currentModule) {
      ...
      findingsHtml += `<div class="module-title">${escapeHtml(currentModule)}</div>`;
    }
    const detail = escapeHtml(f.detail);
    const reco = escapeHtml(f.recommendation);
    const cweTag = f.cwe ? `<span class="taxo-tag taxo-cwe">${escapeHtml(f.cwe)}</span>` : '';
    const owaspTag = f.owasp ? `<span class="taxo-tag taxo-owasp">${escapeHtml(f.owasp)}</span>` : '';
    findingsHtml += `
      <div class="finding">
        <div class="finding-header">
          <span class="badge" style="background:${sevColors[f.severity] || '#888'}">${escapeHtml(sevLabels[f.severity] || f.severity)}</span>
...
<head>
  ...
  <title>VICE Report - ${escapeHtml(hostname)}</title>
  <style>
    :root {
      --primary: #995ff6;
      --accent: #ee967a;
...
<body>
  <div class="container">
    <div class="header">
      ...
      <div class="target">${escapeHtml(hostname)}</div>
    ...
    <div class="footer">
      Generated by <a href="https://github.com/Webba-Creative-Technologies/vice">VICE</a> v${ENGINE_VERSION}<br>
